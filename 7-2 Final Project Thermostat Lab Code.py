...
class TemperatureMachine(StateMachine):
    ...
    StateMachine.redLight = redLight
    StateMachine.blueLight = blueLight
    ##
    ...
    def on_enter_heat(self):
        self.redLight.blink(on_time=0.5, off_time=0.5)
        ##
    ...
    def on_enter_cool(self):
        self.blueLight.blink(on_time=0.5, off_time=0.5)
        ##
        ## TODO: Add the single line of code necessary to update the
        ## lights on the thermostat. 
        ## Remove this TODO comment block when complete.
    ...
    def updateLights(self):
        ...
        ## Determine which LED is used based on state and temperature
        if self.current_state.id == "heat":
            if temp < self.setPoint:    ## If below set point, turn on heating indicator
                self.redLight.blink(on_time=0.5, off_time=0.5)
            else:
                self.redLight.on()    ## Red light is solid when at or above set point

        elif self.current_state.id == "cool":
            if temp > self.setPoint:    ## If above set point, turn on cooling indicator
                self.blueLight.blink(on_time=0.5, off_time=0.5)
            else:
                self.blueLight.on()    ## Blue light is solid when at or below set point

        else:    ## If state is "off", both LEDs are off
            self.redLight.off()
            self.blueLight.off()
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                ## Display thermostat state and setPoint
